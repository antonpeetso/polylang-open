<?php
// Built using https://github.com/grappler/polylang-slug

// Check if PLL exists & the minimum version is correct.
if ( ! defined( 'POLYLANG_VERSION' ) || version_compare( constant('POLYLANG_VERSION'), '1.7', '<=' ) || version_compare( $GLOBALS[ 'wp_version' ], '4.0', '<=' ) ) {
	add_action( 'admin_notices', 'polylang_open_slug_admin_notices' );
	return;
}
 ...
 * @since 0.2.0
 */
function polylang_open_slug_admin_notices() {
 ...
 */
function polylang_open_slug_unique_slug_in_language( $slug, $post_ID, $post_status, $post_type, $post_parent, $original_slug ){
	...
	$join_clause  = polylang_open_slug_model_post_join_clause();
	// " AND pll_tr.term_taxonomy_id IN (" . implode(',', $languages) . ")".
	$where_clause = polylang_open_slug_model_post_where_clause( $lang );
...
add_filter( 'wp_unique_post_slug', 'polylang_open_slug_unique_slug_in_language', 10, 6 );
 ...
 */
function polylang_open_slug_filter_queries( $query ) {
	global $wpdb;
	...
	$is_pages_sql = preg_match(
		...
		polylang_open_slug_standardize_query( $query ),
		$matches
	);
	...
	// Check if should contine. Don't add $query polylang_open_slug_should_run() as $query is a SQL query.
	if ( ! polylang_open_slug_should_run() ) {
		return $query;
	}
	 ...
	 */
	$query = apply_filters( 'polylang_open_slug_sql_query', $sql_query, $matches, $join_clause, $where_clause );


	return $query;
}
add_filter( 'query', 'polylang_open_slug_filter_queries' );
 ...
 */
function polylang_open_slug_posts_where_filter( $where, $query ) {
	// Check if should contine.
	if ( ! polylang_open_slug_should_run( $query ) ) {
		return $where;
	}
	...
	// " AND pll_tr.term_taxonomy_id IN (" . implode(',', $languages) . ")"
	$where .= polylang_open_slug_model_post_where_clause( $lang  );

	return $where;
}
add_filter( 'posts_where', 'polylang_open_slug_posts_where_filter', 10, 2 );
 ...
 */
function polylang_open_slug_posts_join_filter( $join, $query ) {
	...
	// " INNER JOIN $wpdb->term_relationships AS pll_tr ON pll_tr.object_id = ID".
	$join .= polylang_open_slug_model_post_join_clause();

	return $join;
}
add_filter( 'posts_join', 'polylang_open_slug_posts_join_filter', 10, 2 );
 ...
 * @return bool
 */
function polylang_open_slug_should_run( $query = '' ) {
	...
	$disable = apply_filters( 'polylang_open_slug_disable', false, $query );
 ...
 */
function polylang_open_slug_standardize_query( $query ) {
 ...
 * @return string
 */
function polylang_open_slug_model_post_join_clause() {
 ...
 * @return string
 */
function polylang_open_slug_model_post_where_clause( $lang = '' ) {
	if ( function_exists( 'PLL' ) ) {
